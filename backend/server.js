...
const generateToken = (user) => {
  return Buffer.from(`${user.username}:${user.role}:${Date.now()}`).toString('base64');
};
...
server.post('/api/attendance', authenticate, (req, res) => {
  ...
  const existingAttendance = db.get('attendance')
...
server.get('/api/attendance/me', authenticate, (req, res) => {
  const db = router.db;
  const userAttendance = db.get('attendance')
...
server.get('/api/leaves/me', authenticate, (req, res) => {
  const db = router.db;
  const userLeaves = db.get('leaves')
    .filter({ userId: req.user.username })
    .value();
